...
# ---------------------------------------------------------
# 1. 공통 연산 정의 및 그래프 초기화 (YOLOv5-s와 동일)
# ---------------------------------------------------------
OPS = {
...
# ---------------------------------------------------------
# 3. 아키텍처 구성 (YOLOv5-x Scaling 적용)
# ---------------------------------------------------------
# YOLOv5-x Depth Factors (n):
# Backbone C3 blocks: [4, 8, 8, 4] (Scaled by 1.33)
# Neck C3 blocks: [4, 4, 4, 4]

# === Backbone ===
stem = make_conv_module(input_id=-1, k=3, s=2) 

# P2: Conv(s=2) -> C3(n=4)  <-- Increased depth
p2_conv = make_conv_module(stem, k=3, s=2)
p2_out = make_c3(p2_conv, n=4, shortcut=True) # Feature P2

# P3: Conv(s=2) -> C3(n=8)
p3_conv = make_conv_module(p2_out, k=3, s=2)
p3_out = make_c3(p3_conv, n=8, shortcut=True) # Feature P3

# P4: Conv(s=2) -> C3(n=8)
p4_conv = make_conv_module(p3_out, k=3, s=2)
p4_out = make_c3(p4_conv, n=8, shortcut=True) # Feature P4

# P5: Conv(s=2) -> C3(n=4) -> SPPF
p5_conv = make_conv_module(p4_out, k=3, s=2)
p5_c3 = make_c3(p5_conv, n=4, shortcut=True)
p5_out = make_sppf(p5_c3) # Feature P5

# === Neck (PANet/FPN) ===
# 1. Upsample P5 -> Concat P4 -> C3(n=4)  <-- Increased depth
up_p5 = add_node("Upsample_2x", [p5_out])
concat_p4 = add_node("Concat", [up_p5, p4_out])
c3_p4_neck = make_c3(concat_p4, n=4, shortcut=False) 

# 2. Upsample P4_neck -> Concat P3 -> C3(n=4)
up_p4 = add_node("Upsample_2x", [c3_p4_neck])
concat_p3 = add_node("Concat", [up_p4, p3_out])
c3_p3_out = make_c3(concat_p3, n=4, shortcut=False) # Head Input 0 (Small)

# 3. Downsample P3_neck -> Concat P4_neck -> C3(n=4)
down_p3 = make_conv_module(c3_p3_out, k=3, s=2)
concat_p4_2 = add_node("Concat", [down_p3, c3_p4_neck])
c3_p4_out = make_c3(concat_p4_2, n=4, shortcut=False) # Head Input 1 (Medium)

# 4. Downsample P4_neck_out -> Concat P5 -> C3(n=4)
down_p4 = make_conv_module(c3_p4_out, k=3, s=2)
concat_p5_2 = add_node("Concat", [down_p4, p5_out])
c3_p5_out = make_c3(concat_p5_2, n=4, shortcut=False) # Head Input 2 (Large)

# === Head (Coupled Detect) ===
# Single 1x1 Conv output projection at each scale.
head_inputs = [c3_p3_out, c3_p4_out, c3_p5_out]

for h_in in head_inputs:
    # Final output projection
    final_output = add_node("Conv_1x1", [h_in])
    # Placeholder Sigmoid for consistency
